...
public class Enemy : MonoBehaviour
{
    ...
    [Tooltip("Prefab de la roca que se va a disparar")]
    public GameObject rockPrefab;
    [Tooltip("Velocidad del ataque(segundo entre ataques)")]
    ...
    void Update()
    {
        ...
        if (target != initialPosition && distance < attackRadius)
        {
            ...
            anim.Play("Enemy1_Walk", -1, 0);  // Congela la animación de andar
    ...
    // Para que el enemigo ataque al personaje principal
    IEnumerator Attack(float seconds)
    {
        attacking = true; //Para activar la bandera
        if (target != initialPosition && rockPrefab != null)
        {
            Instantiate(rockPrefab, transform.position, transform.rotation);
            //El tiempo ha esperar para que realizar otro ataque
            yield return new WaitForSeconds(seconds);
        }
        attacking = false;
    }
    //Para restarle vida al personaje si este es atacado con la espada
    ...
    //Para poner la vida del enemigo
    private void OnGUI()
    {
        //Guarda la posicion del objeto con respecto al enemigo
        Vector2 pos = Camera.main.WorldToScreenPoint(transform.position);

        //Dibujar el cuadro del enemigo

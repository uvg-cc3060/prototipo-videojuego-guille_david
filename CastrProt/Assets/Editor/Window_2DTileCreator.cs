...
public class Window_2DTileCreator : EditorWindow
{
        ...
        if (setupStage == 3 && tileTextures != null)
        {
            ...
            if (mouseDown)
            {
                ...
                else
                {
                    ...
                    ObtainTileBelow();
                    CheckTile(lowerTileHit, middleTileHit, aboveTileHit, currentTabList[tileSelectedID].transform.Find("Editor Hitbox").gameObject.layer);
                    removingTile = false;
                }
            }
            else
    ...
    void AddTile()
    {
        ...
        metaTile.transform.Find("Editor Hitbox").gameObject.SetActive(true);
        firstClick = 0;
        #endregion
    }
            ...
            for (int y = 0; y < massTileHeight; y++)
            {
                for (int x = 0; x < massTileWidth; x++)
                {
                    if (tileSelected)
                    {
                        ...
                        TileObject.transform.Find("Editor Hitbox").gameObject.SetActive(true);
